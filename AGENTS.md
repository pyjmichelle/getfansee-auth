...
**Latest migration**: `055_lock_paywall_write_access.sql` (`050` age assurance, `051` payment ledger, `052` settlement + reversal + reconciliation, `053` atomic subscription purchase — debit and `subscriptions` row in one transaction, `054` one-time browser-bound claim for the age gate cookie, `055` drops the fan-facing INSERT/UPDATE/DELETE policies on `subscriptions`/`purchases`/`post_unlocks` so paid content can no longer be self-granted with the browser anon key — writes go through SECURITY DEFINER RPCs / the service-role admin client)
