...
- Wallet / Payments (`app/me/wallet/`, `app/api/wallet/`, **PayRam — the MVP rail**: `lib/payram.ts`, `app/api/payments/payram/*`, `app/api/webhooks/payram/`; ledger + settlement in `migrations/051`/`052`, `lib/settlement.ts`, `/api/cron/settlement`, `/api/admin/refunds`. Creator withdrawals: `lib/payouts.ts`, `migrations/056`, `/api/creator/payout-methods`, `/api/creator/withdrawals`, `/api/admin/withdrawals`. Funding and spend share `lib/payments-live.ts` (`arePaymentsLive`). Stripe fiat is disabled by default (`STRIPE_FIAT_ENABLED`); NowPayments remains as the earlier crypto path — see payments-risk agent)
...
**Latest migration**: `058_payout_rpc_hardening.sql` (`050` age assurance, `051` payment ledger, `052` settlement + reversal + reconciliation, `053` atomic subscription purchase — debit and `subscriptions` row in one transaction, `054` one-time browser-bound claim for the age gate cookie, `055` drops the fan-facing INSERT/UPDATE/DELETE policies on `subscriptions`/`purchases`/`post_unlocks` so paid content can no longer be self-granted with the browser anon key — writes go through SECURITY DEFINER RPCs / the service-role admin client, `056` adds creator payout methods + withdrawal requests + `request_withdrawal`/`decide_withdrawal`, and keeps `creator_ledger` payout rows in `available` so identity #3 still balances, `058` rejects non-positive `increment_wallet_available` and `REVOKE`s payout RPCs from `anon`/`authenticated`)
