...
async function seedDemo() {
  ...
  try {
    ...
    if (listError && listError.code !== 'PGRST116') { // PGRST116 = no rows returned
    ...
    for (const post of posts) {
      ...
      console.log(`✅ Created post: ${insertedPost.id} - "${post.title}" (${post.price_cents === 0 ? 'subscriber-only' : `$${(post.price_cents / 100).toFixed(2)} PPV`})`)
    }
