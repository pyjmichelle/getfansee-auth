...
- **创作者提现**（`migrations/056`）：`request_withdrawal` 立刻扣 available 并写负数 ledger（state 保持 `available`）；`decide_withdrawal` 的 `paid` 不得把 ledger 改成 `paid`，否则恒等式 #3 永久失衡。最低 $20。写路径 service_role，表只 SELECT own
