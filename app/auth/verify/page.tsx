...
export default function VerifyPage() {
  ...
  useEffect(() => {
    const handleVerification = async () => {
      try {
        ...
        if (code) {
          console.log("[verify] Processing code exchange")
        ...
        if (token && type) {
          console.log("[verify] Processing email verification token", { type })
          ...
          if (verifyResult.error) {
            ...
            if (verifyResult.error.message.includes("expired") || verifyResult.error.message.includes("invalid")) {
              setMessage("Verification link has expired or is invalid. Please request a new one.")
            } else {
  ...
  const handleResend = async () => {
    ...
    try {
      console.log("[verify] Resending verification email to:", email)
        ...
        <div className="bg-card border border-border rounded-lg p-8 text-center">
          ...
          {status === "success" && (
            <>
              ...
              <h1 className="text-2xl font-semibold text-foreground mb-3">Verification Successful!</h1>
