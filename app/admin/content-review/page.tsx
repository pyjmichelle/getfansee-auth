...
export default function ContentReviewPage() {
  ...
  const getVisibilityBadge = (visibility: string, priceCents: number | null) => {
    if (visibility === "free") {
      ...
      return (
        <Badge className="bg-[var(--wine-tint)] text-wine-text border-[var(--border-wine)]/20 rounded-lg">
          Subscribers
        </Badge>
      );
    } else {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-h1 text-text-primary mb-2">Content Review</h1>
      ...
      <div className="bento-grid mb-6">
        <div className="bento-2x1 card-block p-5">
          <p className="text-small text-text-tertiary">Total Items</p>
          <p className="text-3xl font-bold text-text-primary">{posts.length}</p>
        </div>
        <div className="card-block p-5">
          <p className="text-small text-text-tertiary">Needs Review</p>
          <p className="text-3xl font-bold text-wine-text">{posts.length}</p>
        </div>
        <div className="card-block p-5">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <Link href={`/creator/${post.creator_id}`}>
                      <h3 className="text-lg font-semibold text-text-primary hover:text-wine-text transition-colors">
                  ...
                  {post.title && (
                    <h4 className="text-body-lg font-medium text-text-primary mb-2">
                      {post.title}
                    </h4>
                  )}
                  <p className="text-small text-text-tertiary mb-4 line-clamp-2">{post.content}</p>
                  ...
                  <div className="flex gap-3 pt-4 border-t border-border-base">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="border-border-base bg-surface-base hover:bg-surface-raised rounded-xl active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-[var(--wine)]"
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="border-error text-error hover:bg-error/10 rounded-xl active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-error"
                    >
