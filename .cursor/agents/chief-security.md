...
- **提现表同样只给 SELECT own**（2026-09-19 `migrations/056` + `058`）：`creator_payout_methods` / `withdrawal_requests` 禁止 fan/creator 直写。写入走 `request_withdrawal` / `decide_withdrawal`（`SECURITY DEFINER`，仅 `service_role`）或 admin 客户端。创作者不能自己把一笔提现标成 `paid`。`058` 对这两支 RPC 和 `increment_wallet_available` 从 `anon`/`authenticated` 显式 `REVOKE`，并禁止负数自增
